...
if (!function_exists('time_elapsed_string')) {
    function time_elapsed_string($datetime, $full = false) {
        ...
        // Adding weeks calculation directly into the string generation logic
        if ($diff->d >= 7) {
...
if (!function_exists('getMostClickedPost')) {
    function getMostClickedPost() {
        global $con;

        $query = "SELECT post_id, COUNT(post_id) AS click_count
                  FROM post_clicks
                  GROUP BY post_id
...
if(!function_exists('create_tables')){
    ...
    */
        ...
        $query = "CREATE TABLE IF NOT EXISTS comment_log (
            id INT PRIMARY KEY AUTO_INCREMENT,
            comment_id INT,
            user_id INT,
            action VARCHAR(50),
