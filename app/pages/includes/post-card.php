<div class="col-md-6">
    <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm position-relative" style="width: 100%;">
        ...
        <?php if($row['user_id'] == user('id')) { ?>
            <a href="<?=ROOT?>pages/edit.php?id=<?=$row['id']?>" class="btn btn-sm btn-primary position-absolute bottom-0 start-0 mt-5" style="width: 20%;">Edit</a>
        <?php } ?>
        ...
        <div class="col p-4 d-flex flex-column position-static" style="width: calc(100% - 340px);">
            ...
            <!-- Truncated Content -->
            <p class="card-text mb-auto" id="truncatedContent"><?=substr($row['content'],0,30)?></p>
            ...
            <?php if(strlen($row['content']) > 50) { ?>
                <a href="#" class="read-more" style="width: 60px; display: inline-block; background-color: #007bff; color: #fff; border: none; padding: 8px 20px; text-align: center; text-decoration: none; border-radius: 4px; cursor: pointer; transition: background-color 0.3s ease;">Read more</a>
            <?php } ?>
        </div>
    </div>
</div>
