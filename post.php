<?php 
    ...
    if(!empty($_POST)){
      ...
      if ($result) {
        $logQuery = "INSERT INTO comment_log (user_id, post_id, action) VALUES (:user_id, :post_id, 'comment_posted')";
        query($logQuery, ['user_id' => $user_id, 'post_id' => $post_id]);
      }
...
function fetchComments(postId, lastDate) {
    $.ajax({
        url: 'check-comment.php',
        type: 'GET',
        data: { post_id: postId, last_date: lastDate },
        success: function(comments) {
            if (comments.length > 0) {
                lastDate = new Date().toISOString(); // Update lastDate with new timestamp
